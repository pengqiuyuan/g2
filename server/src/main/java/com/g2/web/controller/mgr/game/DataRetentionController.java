...
import com.g2.service.game.DataRetentionService;
 ...
 *
 */
@Controller("dataRetentionController")
@RequestMapping(value="/manage/game/dataRetention")
public class DataRetentionController extends BaseController{
	...
	public static void setSortTypes(Map<String, String> sortTypes) {
		DataRetentionController.sortTypes = sortTypes;
	}
	...
	@Autowired
	private DataRetentionService dataRetentioService;
	...
	public String index(@RequestParam(value = "page", defaultValue = "1") int pageNumber,
		...
		Long userId = dataRetentioService.getCurrentUserId();
		User user = accountService.getUser(userId);

		model.addAttribute("user", user);
		model.addAttribute("dateFrom", dataRetentioService.thirtyDayAgoFrom());
		model.addAttribute("dateTo", dataRetentioService.nowDate());
		...
		logService.log(dataRetentioService.getCurrentUser().getName(), dataRetentioService.getCurrentUser().getName() + "：查询留存率页面", Log.TYPE_DATA_RETENTION);
		// 将搜索条件编码成字符串，用于排序，分页的URL
		model.addAttribute("searchParams", Servlets.encodeParameterStringWithPrefix(searchParams, "search_"));
		return "/game/dataretention/index";
	}
